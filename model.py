...
class Resnet_H36m(nn.Module):
    ...
    def forward(self, x, x_small, Ks=None, position_gt=None, scale_gt=None, rectangular_images=False):
        if not self.use_pcl:
            ...
            # De-normalize
            mean = self.dataset_cfg.get_joint_mean().to(self.device)
            std = self.dataset_cfg.get_joint_std().to(self.device)
            new_pre_transform = utils.denorm_human_joints(new_pre_transform, mean, std).unsqueeze(3)
            ...
            # Normalize again
            normalized_new_output = utils.batch_normalize_canon_human_joints(new_output, mean, std)
            new_pre_transform = new_pre_transform.view(x.shape[0], self.num_joints, 3)
            new_pre_transform = utils.batch_normalize_canon_human_joints(new_pre_transform, mean, std)
