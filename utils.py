...
def batch_normalize_canon_human_joints(joints, mean, std):
    """
    Normalize 2D or 3D joints (shape: [B, 17, C]) using canonical per-joint mean/std.
    Automatically handles both 2D (C=2) and 3D (C=3).
    Skips pelvis joint (index 14).
    """
    assert joints.shape[1] == 17, "Expected 17 joints"
    assert joints.shape[2] in [2, 3], "Expected 2D or 3D joints"
    batch_size = joints.shape[0]
    C = joints.shape[2]

    mean = mean.unsqueeze(0).expand(batch_size, -1, -1)  # [B, 17, C]
    std = std.unsqueeze(0).expand(batch_size, -1, -1)

    normalized_joints = joints.clone()
    normalized_joints[:, :14, :] = (joints[:, :14, :] - mean[:, :14, :]) / std[:, :14, :]
    normalized_joints[:, 15:, :] = (joints[:, 15:, :] - mean[:, 15:, :]) / std[:, 15:, :]
    return normalized_joints
